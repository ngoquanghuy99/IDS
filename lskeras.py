...
checkpointer = callbacks.ModelCheckpoint(filepath="results/1layer/checkpoint-{epoch:02d}.hdf5", verbose=1, save_best_only=True, monitor='val_acc',mode='max')
...
model.save("results/1layer/models/1layer_model.hdf5")

loss, accuracy = model.evaluate(X_test, y_test)
print("\nLoss: %.2f, Accuracy: %.2f%%" % (loss, accuracy*100))
y_pred = model.predict_classes(X_test)
np.savetxt('results/1layer/predictions.txt', np.transpose([y_test1,y_pred]), fmt='%01d')
